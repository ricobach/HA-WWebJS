...
class WWebJSApi:
        ...
        *,
        ...
        try:
            ...
            raise WWebJSApiError(
                f"WWebJS API returned HTTP {err.status}"
            ) from err
        except (ClientError, TimeoutError) as err:
            raise WWebJSConnectionError(
                f"Unable to connect to WWebJS API at {self._base_url}"
            ) from err
    ...
    async def request_pairing_code(
        ...
        data = await self._request(
            "POST",
            f"/session/requestPairingCode/{session_id}",
            json_data={
        ...
        if isinstance(code, dict):
            code = (
                ...
                or code.get("pairing_code")
                or code.get("code")
            )
        if not code:
            raise WWebJSApiError(
                "The API did not return a pairing code in a recognized format"
            )
        return str(code)
    ...
    async def get_message_info(
        ...
        return await self._request(
            "POST",
            f"/message/getInfo/{session_id}",
            json_data={"chatId": chat_id, "messageId": message_id},
        )
...
def normalize_target(value: str) -> str:
    ...
    if not digits:
        raise WWebJSApiError(
            "Target must contain a phone number or WWebJS chat ID"
        )
